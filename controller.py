...
class SingleActionController():
    def __init__(self, config, ckpt_name):
        ...
        # set time to run
        self.max_timesteps = int(max_timesteps * 1) # may increase for real-world tasks
        ...
        with torch.inference_mode():
            ...
            for t in range(self.max_timesteps):
                ...
                if t % self.query_frequency == 0:
                    ...
                    all_actions = torch.cat([all_actions[:, :-self.BASE_DELAY, :-2], all_actions[:, self.BASE_DELAY:, -2:]], dim=2)
...
# def single_action_controller(config, ckpt_name):
#     set_seed(1000)
#     ckpt_dir = config['ckpt_dir']
#     policy_config = config['policy_config']
...
#             ### step the environment
#             ts = robot.step(target_qpos, base_action)
...
#         move_grippers([robot.puppet_bot_left, robot.puppet_bot_right], [PUPPET_GRIPPER_JOINT_OPEN] * 2, move_time=0.5)  # open
...
if __name__ == "__main__":
    sac = SingleActionController(controller_config, 'test')
    sac.run()